...
def knife(L):
    ...
    cx=Wd//2
    POM_R = L*0.036
    GRIP0, GRIP1 = L*0.055, L*0.170          # grip runs down to the guard
    GY0, GY1     = L*0.170, L*0.205          # crossguard band
    BL0          = L*0.205                   # blade starts
    bw           = L*0.052                   # blade half-width at the guard
    # blade: light face, shadow face, and a fuller down the middle
    ...
    kd.polygon([(cx-bw*0.30,BL0+L*0.012),(cx+bw*0.30,BL0+L*0.012),
                (cx+bw*0.14,L*0.855),(cx-bw*0.14,L*0.855)], fill=(176,186,201,255))
    ...
    # grip
    kd.rounded_rectangle([cx-L*0.030,GRIP0,cx+L*0.030,GRIP1],int(L*0.016), fill=(43,34,31,255))
    kd.rounded_rectangle([cx-L*0.030,GRIP0,cx-L*0.010,GRIP1],int(L*0.010), fill=(68,55,48,255))
    for i in range(5):
        yy = GRIP0 + L*0.012 + i*(GRIP1-GRIP0-L*0.020)/5
        kd.rounded_rectangle([cx-L*0.030,yy,cx+L*0.030,yy+L*0.008],int(L*0.004), fill=(58,46,41,255))
    # pommel
    kd.ellipse([cx-POM_R,GRIP0-POM_R*1.5,cx+POM_R,GRIP0+POM_R*0.5], fill=(88,97,114,255))
    kd.ellipse([cx-POM_R*0.72,GRIP0-POM_R*1.32,cx+POM_R*0.16,GRIP0-POM_R*0.10], fill=(126,137,157,255))
    return k

KN_ANG, KN_LEN = 14, px(500)
def marked(L):
    k=knife(L); dd_=ImageDraw.Draw(k)
    dd_.rectangle([k.width//2-1,L-3,k.width//2+1,L-1], fill=(255,0,255,255))
    dd_.rectangle([k.width//2-1,int(L*0.020),k.width//2+1,int(L*0.020)+2], fill=(0,255,255,255))
    return k
...
TIP    = (DUO_XY[0]+int(duo.width*0.44), DUO_XY[1]+int(duo.height*0.70))
KN_XY  = (TIP[0]-TIP_OFF[0], TIP[1]-TIP_OFF[1])
HDL    = (KN_XY[0]+HDL_OFF[0], KN_XY[1]+HDL_OFF[1])
LOGO_XY= (int(HDL[0]-logo.width*1.02), int(HDL[1]-logo.height*0.12))
...
wd = stamp(px(104), px(26), _wound, ang)
...
out.save("verbavia_sword.png")
port = Image.new("RGB",(1080,1350),(10,10,13))
band = out.resize((1080,int(out.height*1080/out.width)), Image.LANCZOS)
port.paste(band,(0,int(1350*0.5-band.height*0.5))); port.save("verbavia_sword_portrait.png")
